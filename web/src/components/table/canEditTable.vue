...
<template>
    <div>
        <Table :height="height" @on-row-click="onRowClick" :ref="refs" :columns="columnsList" :data="thisTableData" border disabled-hover></Table>
    </div>
</template>
...
const editButton = (vm, h, currentRow, index) => {
    return h('Button', {
        ...
        style: {
            margin: '0 5px',
            display: true || currentRow["id"] == undefined ? '' : 'none'
        },
        on: {
            'click': () => {
                if (!currentRow.editting) {
                    ...
                    vm.$emit('on-change', vm.handleBackdata(vm.thisTableData), index, oldData);
                }
            }
        }
...
export default {
    ...
    methods: {
        ...
        init () {
            ...
            let develop = vm.$store.state.user.access.indexOf('DEVELOP_ADMIN') >= 0;
            ...
            this.columnsList.forEach(item => {
                if (item.editable) {
                    item.render = (h, param) => {
                        ...
                        //开发管理员 编辑所有
						//非开发管理员  编辑状态  editingStatus = 2可编辑key 与描述
